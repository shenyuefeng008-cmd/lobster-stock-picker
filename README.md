...
> A 股量化短线交易 Agent — 从盘前选股到竞价买入，从盘中巡检到收盘复盘，全自动覆盖完整个交易日。

一个基于三维度框架（点·线·面）运行的 Python 量化交易系统，集成多源行情采集、多维评分引擎、模拟持仓管理和微信公众号发布，由 20 个 Cron 任务全天候驱动。
...
| **每日进化** | 每日经验沉淀 → rules.md + MEMORY.md 自动追加 |
| **自动发布** | 微信公众号文章自动发布 + 腾讯乐享知识库同步 |
...
| **获取方式** | 微信公众号后台 → 开发 → 基本配置 |
| **配置位置** | `.env` 文件或 `~/.baoyu-skills/.env` |
| **需要权限** | 草稿管理、素材管理、群发接口 |

```bash
# 写入 .env
echo 'WECHAT_APPID=wx你的APPID' >> .env
echo 'WECHAT_APPSECRET=你的APPSECRET' >> .env
```

> ⚠️ 需要公众号已通过微信认证并开通群发权限。
...
| 项目 | 说明 |
|------|------|
| **依赖** | `mcporter` CLI + lexiang MCP |
| **用途** | 复盘报告同步到企业知识库 |

```bash
# mcporter CLI 已预配，确认 lexiang MCP 已连接即可
mcporter call lexiang status
```
