...
if os.path.exists(persist):
    ...
    print(f'✅ premarket_candidates 从持久化副本恢复: {persist}')
elif not os.path.exists(tmp_file):
    print(f'⚠️ premarket_candidates 无持久化副本，/tmp 也不存在')
else:
    print(f'⏭️ /tmp 已有 premarket_candidates，跳过恢复')
...
if os.path.exists(persist_bid):
    ...
    print(f'✅ bid_result 从持久化副本恢复: {persist_bid}')
elif not os.path.exists(tmp_bid):
    print(f'⚠️ bid_result 无持久化副本，/tmp 也不存在')
else:
    print(f'⏭️ /tmp 已有 bid_result，跳过恢复')
"
```
...
if up_count > 3500 and (pre_up is None or pre_up <= 3500):
    alerts.append("🔴 午盘极度高潮：涨跌家数>3500，切辅助模式，仓位上限20%")
...
## 步骤7.5：午间情绪恶化卖出触发（v6.1新增）

> **目的**：填补午间11:30-13:00 30分钟空窗风险，当情绪处于极端区间时立即触发卖出检测
...
try:
    with open('/tmp/lobster_midday_data.json') as f:
        midday = json.load(f)
    up_count = midday.get('up_count', 0)
except:
    print("⚠️ 无法读取午间数据，跳过卖出检测")
    sys.exit(0)

# 仅当情绪处于冰点(<1600)或极度高潮(>3500)时触发卖出
if up_count < 1600:
    print(f"🚨 午间冰点({up_count}涨)，触发卖出检测")
    trigger_sell = True
elif up_count > 3500:
    print(f"🔴 午间极度高潮({up_count}涨)，触发卖出检测")
    trigger_sell = True
else:
    print(f"✅ 情绪正常({up_count}涨)，跳过卖出检测")
    trigger_sell = False

if trigger_sell:
    # 直接运行完整巡检以触发卖出（脚本内置 is_trading_hours 检查，
    # 11:30触发时需绕过时间检查，使用环境变量跳过）
    import subprocess
    patrol_script = os.path.expanduser('~/.qclaw/workspace-1gwpiwf3hr163jz5/scripts/lobster_intraday_patrol.py')
    env = os.environ.copy()
    env['LOBSTER_FORCE_RUN'] = '1'  # 强制运行标志
    ...
    if result.returncode != 0:
        print(f"⚠️ 卖出检测执行异常: {result.stderr[:500]}")
else:
    print("当前情绪不在极端区间，无需午间卖出")
PYEOF
```

**成功标志**：输出卖出触发结果或"情绪正常跳过"
